...
const _AICoserPromptIdToText = new Map();
let _AICoserQueueHint = null;

async function _AICoserFetchQueueInfo() {
    try {
...
async function _AICoserClearBackendQueue() {
    try {
        if (api && typeof api.fetchApi === "function") {
            const r = await api.fetchApi("/queue", {
                method: "POST",
...
function _AICoserSummarizePromptText(promptId, maxLen = 2200) {
    try {
        const t = _AICoserPromptIdToText.get(String(promptId));
...
async function _AICoserQueuePromptWithHint(promptObj, hintText) {
    try {
        _AICoserQueueHint = typeof hintText === "string" ? hintText : null;
        const resp = await api.queuePrompt(0, promptObj);
        const pid = resp?.prompt_id;
        if (pid && typeof hintText === "string") {
            _AICoserPromptIdToText.set(String(pid), hintText);
        }
        return resp;
    } finally {
        _AICoserQueueHint = null;
    }
}

function _AICoserTraceAllEnabled() {
    try {
        return !!(typeof window !== "undefined" && window && window.__AICOSER_TRACE_ALL);
    } catch {
        return false;
    }
}

function _AICoserSafeStringify(obj) {
    try {
        const MAX_STR = 4000;
...
try {
    if (!Object.__AICOSERPatchedEntries) {
        ...
        Object.__AICOSERPatchedEntries = true;
...
try {
    if (app && typeof app.registerExtension === "function" && !app.__AICOSERPatchedRegisterExtension) {
        ...
        app.registerExtension = function (ext) {
            try {
                if (ext && typeof ext === "object") {
                    ...
                    if (typeof ext.getCustomWidgets === "function" && !ext.getCustomWidgets.__AICOSERWrapped) {
                        const prevGet = ext.getCustomWidgets.bind(ext);
                        ext.getCustomWidgets = function () {
                            const r = prevGet.apply(this, arguments);
                            return r && typeof r === "object" ? r : {};
                        };
                        ext.getCustomWidgets.__AICOSERWrapped = true;
                    }
                }
            } catch {
                // ignore
        ...
        app.__AICOSERPatchedRegisterExtension = true;
        ...
        if (Array.isArray(exts)) {
            for (const e of exts) {
                if (e && typeof e === "object") {
                    ...
                    if (typeof e.getCustomWidgets === "function" && !e.getCustomWidgets.__AICOSERWrapped) {
                        const prevGet = e.getCustomWidgets.bind(e);
                        e.getCustomWidgets = function () {
                            const r = prevGet.apply(this, arguments);
                            return r && typeof r === "object" ? r : {};
                        };
                        e.getCustomWidgets.__AICOSERWrapped = true;
                    }
                }
            }
        }
...
try {
    ...
    if (svc && typeof svc.registerExtension === "function" && !svc.__AICOSERPatchedRegisterExtension) {
        ...
        svc.__AICOSERPatchedRegisterExtension = true;
...
try {
    if (!window.__AICOSERPatchedRegisterCustomWidgets) {
        const wrapRegisterCustomWidgets = (obj, path) => {
            if (!obj || typeof obj !== "object") return 0;
            const fn = obj.registerCustomWidgets;
            if (typeof fn !== "function" || fn.__AICOSERWrapped) return 0;
            ...
            obj.registerCustomWidgets.__AICOSERWrapped = true;
        ...
        window.__AICOSERPatchedRegisterCustomWidgets = true;
    }
} catch {
    // ignore
}

const _AICoserGlobalSequentialNodeIds = new Set();
let _AICoserGlobalQueueInstalled = false;
let _AICoserQueuePromptPrev = null;
let _AICoserQueuePromptInHook = false;
let _AICoserApiQueuePromptPrev = null;
let _AICoserApiQueuePromptInHook = false;
let _AICoserInGlobalQueuePrompt = false;
let _AICoserGlobalQueueBtnInstalled = false;
let _AICoserInGlobalQueueBtnClick = false;
let _AICoserGlobalBtnDebugLastTsCandidate = 0;
let _AICoserGlobalBtnDebugLastTsSkip = 0;
let _AICoserGlobalBtnLockUntilTs = 0;
const _AICoserPendingAutoExpandNodeIds = new Set();

let _AICoserExecutedHandler = null;
...
function _wrapPromptQueueNodeOnExecuted(node) {
    try {
        ...
        if (typeof prev === "function" && prev.__AICOSERWrappedPromptQueueNodeOnExecuted) {
            return;
        }
        ...
        const wrapped = function () {
            try {
                ...
                if (typeof _AICoserExecutedHandler === "function") {
                    _AICoserExecutedHandler({ detail: { node: node?.id, display_node: node?.id, output: outArg } }, "PromptQueueNode.onExecuted");
                }
            } catch {
                // ignore
            }
        ...
        wrapped.__AICOSERWrappedPromptQueueNodeOnExecuted = true;
...
let _AICoserExecutedHookInstalled = false;
function _matchPendingByApiNodeId(nodeId, displayNodeId) {
    try {
        ...
        for (const pendingId of _AICoserPendingAutoExpandNodeIds) {
...
function _installExecutedEventHookOnce() {
    if (_AICoserExecutedHookInstalled) return;
    try {
        let dbgLastEvtTs = 0;
        let dbgCount = 0;
        ...
        const dbgLogEvt = (tag, name, evtOrDetail) => {
            try {
                const traceAll = _AICoserTraceAllEnabled();
                const now = Date.now();
                const pendingCount = _AICoserPendingAutoExpandNodeIds?.size || 0;
                ...
                if (evtLower === "crystools.monitor") {
                    const forced0 = !!(typeof window !== "undefined" && window && window.__AICOSER_EXEC_EVT_DEBUG);
                    if (!forced0) return;
                }
                ...
                const forced = !!(typeof window !== "undefined" && window && window.__AICOSER_EXEC_EVT_DEBUG);
                if (!traceAll && !forced) {
                    // Find the latest run context ts among pending nodes.
                    let latestTs = 0;
                    try {
                        for (const nid of _AICoserPendingAutoExpandNodeIds) {
                ...
                if (traceAll) {
                    console.info("[PromptQueue] exec-event sniff (json)", _AICoserSafeStringify({
                        tag,
                        evtType,
                        pendingCount,
                        detail: sample,
        ...
        const handler = (payload, source) => {
            try {
                ...
                const pendingIds0 = Array.from(_AICoserPendingAutoExpandNodeIds).slice(0, 20);

                const matchedNodeId =
                    (displayNodeId && _AICoserPendingAutoExpandNodeIds.has(displayNodeId) ? displayNodeId : null) ||
                    (nodeId && _AICoserPendingAutoExpandNodeIds.has(nodeId) ? nodeId : null) ||
                    null;
                ...
                if (!matchedNodeId && !mappedPendingId && _AICoserPendingAutoExpandNodeIds.size === 1) {
                    try {
                        const onlyPending = Array.from(_AICoserPendingAutoExpandNodeIds)[0];
                ...
                if (!effectiveNodeId) {
                    ...
                    try {
                        if (_AICoserPendingAutoExpandNodeIds.size === 1) {
                            const pendingId = Array.from(_AICoserPendingAutoExpandNodeIds)[0];
                                ...
                                if (Number.isFinite(num)) {
                                    ...
                                    if (next.mirrorRunId === curRunId && Number.isFinite(next.mirrorIdx) && Number.isFinite(next.mirrorTotal)) {
                                        ...
                                        if (Number.isFinite(expectedTotal) && expectedTotal <= 1) {
                                            _AICoserPendingAutoExpandNodeIds.delete(String(pendingId));
                                        ...
                                        if (idx0 === 0 && total0 > 1) {
                                            ...
                                            if (node) {
                                                ...
                                                _setRunCtx(String(pendingId), next);
                                                _AICoserPendingAutoExpandNodeIds.add(String(pendingId));
                ...
                if (idx !== 0) return;
                if (total <= 1) {
                    _AICoserPendingAutoExpandNodeIds.delete(effectiveNodeId);
        ...
        _AICoserExecutedHandler = handler;
        ...
        const maybeHandleExecutedLike = (evtType, payload, source) => {
            try {
                ...
                console.info("[PromptQueue] executed-like event => handler", {
                    ...
                    source,
                    pendingIds: Array.from(_AICoserPendingAutoExpandNodeIds).slice(0, 20),
        ...
        try {
            const tryInstallLGraphHook = () => {
                try {
                    ...
                    if (proto.__AICOSERPatchedOnExecutedSetter) return true;

                    proto.__AICOSERPatchedOnExecutedSetter = true;
                    const sym = "__AICOSEROrigOnExecuted";
                    ...
                    const makeWrapped = (fn) => {
                        ...
                        if (fn.__AICOSERWrappedOnExecuted) return fn;
                        const wrapped = function () {
                            try {
                                ...
                                try {
                                    if (_AICoserPendingAutoExpandNodeIds.size === 1) {
                                        ...
                                        if (traceAll || !ctx0 || !ctx0.__dbgOnExecTs || now - ctx0.__dbgOnExecTs > 800) {
                                            ...
                                            if (traceAll) {
                                                console.info("[PromptQueue] onExecuted sniff (json)", _AICoserSafeStringify({
                                                    pendingId: String(onlyPending),
                                                    runId: ctx0?.runId || null,
                                                    thisId: nodeId,
                                                    outArg,
                                ...
                                if (nodeId && _AICoserPendingAutoExpandNodeIds.has(nodeId)) {
                                    handler({ detail: { node: this?.id, display_node: this?.id, output: outArg } }, "node.onExecuted");
                                } else if (_AICoserPendingAutoExpandNodeIds.size === 1) {
                        ...
                        wrapped.__AICOSERWrappedOnExecuted = true;
                        return wrapped;
                    };
        ...
        try {
            ...
            if (proto && typeof proto.dispatchEvent === "function" && !proto.__AICOSERPatchedDispatchEvent) {
                proto.__AICOSERPatchedDispatchEvent = true;
        ...
        const installDispatchHook = (obj, name) => {
            try {
                ...
                const flag = `__AICOSERPatchedDispatchCustomEvent_${name}`;
        ...
        _AICoserExecutedHookInstalled = true;
...
let _AICoserRunSeq = 0;
const _AICoserRunCtxByNodeId = new Map();
function _nextRunId() {
    _AICoserRunSeq = (_AICoserRunSeq + 1) % 1000000;
    return `${Date.now()}-${_AICoserRunSeq}`;
}

function _getRunCtx(nodeId) {
    try {
        const key = String(nodeId);
        return _AICoserRunCtxByNodeId.get(key) || null;
    } catch {
        return null;
    }
}
...
function _setRunCtx(nodeId, ctx) {
    try {
        const key = String(nodeId);
        if (!key) return;
        _AICoserRunCtxByNodeId.set(key, ctx);
    } catch {
        // ignore
    }
}
...
function _clearRunCtx(nodeId) {
    try {
        _AICoserRunCtxByNodeId.delete(String(nodeId));
    } catch {
        // ignore
    }
}
...
function _resetNodeAutoExpandState(node, reason) {
    try {
        ...
        console.info("[PromptQueue] reset auto-expand state start", {
            ...
            pendingBefore: _AICoserPendingAutoExpandNodeIds?.has?.(nodeId) || false,
        });
        try {
            _AICoserPendingAutoExpandNodeIds.delete(nodeId);
        } catch {
            // ignore
        }
        _clearRunCtx(nodeId);
        console.info("[PromptQueue] reset auto-expand state ok", {
            nodeId,
            reason: String(reason || ""),
            pendingAfter: _AICoserPendingAutoExpandNodeIds?.has?.(nodeId) || false,
        });
    } catch {
        // ignore
    }
...
async function _autoEnqueueRemainingAfterFirstRun(node, total) {
    if (!node) return;
    const nodeId = String(node.id);
    if (!_AICoserPendingAutoExpandNodeIds.has(nodeId)) return;
    _AICoserPendingAutoExpandNodeIds.delete(nodeId);
    ...
    try {
        for (let idx = 1; idx < total; idx++) {
            ...
            await _AICoserQueuePromptWithHint(p, hint);
        }
    } catch (e) {
...
function _AICoserHookSetter(obj, prop, wrapFactory) {
    try {
...
function _installGlobalQueueButtonClickHookOnce() {
    if (_AICoserGlobalQueueBtnInstalled) return;
    _AICoserGlobalQueueBtnInstalled = true;
    console.info("[PromptQueue] installing global queue button click hook");

    const handler = async (e) => {
            try {
                const nowLock = Date.now();
                if (_AICoserInGlobalQueueBtnClick || nowLock < _AICoserGlobalBtnLockUntilTs) return;
                const enabledNodes = _getEnabledPromptQueueNodes();
                if (enabledNodes.length !== 1) {
                    const now = Date.now();
                    if (now - _AICoserGlobalBtnDebugLastTsSkip > 300) {
                        _AICoserGlobalBtnDebugLastTsSkip = now;
                ...
                if (t?.closest?.("[data-AICoser-promptqueue]")) return;
                ...
                const now0 = Date.now();
                if (now0 - _AICoserGlobalBtnDebugLastTsCandidate > 1200) {
                    _AICoserGlobalBtnDebugLastTsCandidate = now0;
                ...
                if (!_isLikelyGlobalQueueButton(clickable)) {
                    ...
                    const now = Date.now();
                    if (now - _AICoserGlobalBtnDebugLastTsSkip > 1200) {
                        _AICoserGlobalBtnDebugLastTsSkip = now;
                ...
                _AICoserInGlobalQueueBtnClick = true;
                _AICoserGlobalBtnLockUntilTs = Date.now() + 350;
                setTimeout(() => {
                    _AICoserInGlobalQueueBtnClick = false;
                }, 360);
                ...
                try {
                    ...
                    console.info("[PromptQueue] executed hook status", {
                        runId,
                        installed: _AICoserExecutedHookInstalled,
                    });
                } catch {
                    // ignore
                }
                ...
                if (String(node?.id) === "-1") {
                    _AICoserPendingAutoExpandNodeIds.add(String(node.id));
...
function _installGlobalSequentialQueueHookOnce() {
    if (_AICoserGlobalQueueInstalled) return;
    _AICoserGlobalQueueInstalled = true;

    _installGlobalQueueButtonClickHookOnce();

    _installExecutedEventHookOnce();

    _AICoserHookSetter(api, "queuePrompt", (getCurrent) =>
        async function () {
            const cur = getCurrent();
            _AICoserApiQueuePromptPrev = cur;
            if (_AICoserApiQueuePromptInHook) {
            ...
            try {
                if (_AICoserTraceAllEnabled()) {
                    ...
                    console.info("[PromptQueue] api.queuePrompt prompt.output snapshot (json)", _AICoserSafeStringify({
                        runId,
                        nodeId: nodeIdRun,
                        nodes,
                    }));
            ...
            if (hasUpstreamPrompts) {
                ...
                if (Array.isArray(pv0) && pv0.length > 1) {
                    ...
                    if (indices.length > 1) {
                        ...
                        _AICoserApiQueuePromptInHook = true;
                        const t0 = performance?.now?.() ?? Date.now();
                        ...
                        try {
                            ...
                            _AICoserApiQueuePromptInHook = false;
                        }
                    }
                }
    ...
    _AICoserHookSetter(app, "queuePrompt", (getCurrent) =>
        async function () {
            const cur = getCurrent();
            _AICoserQueuePromptPrev = cur;
            if (_AICoserQueuePromptInHook) return await cur.apply(app, arguments);

            _AICoserQueuePromptInHook = true;
            try {
                _AICoserInGlobalQueuePrompt = true;
                return await cur.apply(app, arguments);
            } finally {
                _AICoserInGlobalQueuePrompt = false;
                _AICoserQueuePromptInHook = false;
            }
        }
    );
}
...
function getPromptQueueMode(node) {
    const fromProps = node?.properties?.AICoserPromptQueueMode;
    if (typeof fromProps === "string" && fromProps) return fromProps;
    const fromRuntime = node?._AICoserPromptQueueMode;
...
async function queueCurrent(node) {
    ...
    await _AICoserQueuePromptWithHint(prompt, "");
}
...
async function queueAllPromptsSequential(node) {
    ...
    for (const idx of indices) {
        ...
        apiNode.inputs.index = idx;
        const hint = typeof items?.[idx] === "string" ? String(items[idx]) : "";
        await _AICoserQueuePromptWithHint(prompt, hint);
    }
}
...
function createPromptQueueUI(node) {
    ...
    const setGlobalEnabled = (enabled, { silent = false } = {}) => {
        ...
        if (globalChk.checked) {
            ...
            _AICoserGlobalSequentialNodeIds.clear();
            if (typeof node?.id === "number" && node.id >= 0) {
                _AICoserGlobalSequentialNodeIds.add(String(node.id));
            }
        } else {
            if (typeof node?.id === "number" && node.id >= 0) {
                _AICoserGlobalSequentialNodeIds.delete(String(node.id));
            }
        }
    };
    ...
    async function refreshBackendQueue() {
        ...
        try {
            const q = await _AICoserFetchQueueInfo();
            ...
            const addRow = (status, item) => {
                ...
                const txt = _AICoserSummarizePromptText(pid);
                rows.push(
                    `<tr>` +
    ...
    backendClearBtn.onclick = async (e) => {
        ...
        const ok = await _AICoserClearBackendQueue();
...
app.registerExtension({
    name: "AICoser.PromptQueue.Extension",
    customWidgets: {},
    async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        if (!nodeType.prototype.__AICOSERHookedOnExecuted) {
            nodeType.prototype.__AICOSERHookedOnExecuted = true;
            console.info("[PromptQueue] installing onExecuted hook via setter");
            _AICoserHookSetter(nodeType.prototype, "onExecuted", (getCurrent) =>
                function (output) {
                    const cur = getCurrent();
                    ...
                    try {
                        ...
                        console.info("[PromptQueue] onExecuted called", {
                            runId: ctx?.runId || null,
                            nodeId: node?.id,
                            hasPending: _AICoserPendingAutoExpandNodeIds.has(nodeId),
                        });
                        if (!_AICoserPendingAutoExpandNodeIds.has(nodeId)) return;
        ...
        nodeType.prototype.onRemoved = function () {
            try {
                _AICoserGlobalSequentialNodeIds.delete(String(this?.id));
                _AICoserPendingAutoExpandNodeIds.delete(String(this?.id));
